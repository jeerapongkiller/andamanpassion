...
if (!empty($_POST["prodval"])) {
    ...
    $query = "UPDATE booking_products SET trash_deleted = ? WHERE id = ?";
    $procedural_statement = mysqli_prepare($mysqli_p, $query);
    mysqli_stmt_bind_param($procedural_statement, 'ii', $trash_deleted, $prodval);
    $trash_deleted = 1;
    ...
    if (!empty($_POST["prodval"]) && !empty($_POST["bookval"])) {
        ...
        $ip = get_client_ip();
        $description_field = "สถานะ : ยกเลิกข้อมูล";
    ...
    // $message .= "<tr>";
    // $message .= "<td colspan='2' align='center'><span style='color:#FF0000; font-size:1.5em;'> รายละเอียดสินค้า ยกเลิก </span> <br/></td>";
    // $message .= "</tr>";
    // $message .= "<tr>";
    ...
    // $message .= "<b>Remark : </b> ยกเลิกสินค้า <br /><br />";
    ...
    // $can_date = strtotime("now");
    // $can_full = "cancel-" . $can_date . "";
    // $can_file = "../../../assets/cancel_pdf/" . $can_full . ".pdf";
    // $can_name = $can_full.".pdf";

    // $mpdf->WriteHTML($message);
    // $mpdf->Output("../../../assets/cancel_pdf/" . $can_full . ".pdf");
    // ob_end_flush();

    // $Content = '
    //     <p style="font-size:1.5em; font-family: sans-serif;"><span style="font-weight: bold; font-size:1.5em;"> ยกเลิกสินค้า </span><br />
    ...
    $query_confirm = "SELECT booking , status_email FROM booking_products WHERE id > 0 
        AND booking = ? AND trash_deleted = 0 AND status_email = 3 ";
    ...
    if ($numrowconfirm == 0) {
        ...
        $status_email_revise = "0";
    }
    $num_email = "2";
    $query_book = "UPDATE booking SET status_email = ? , status_email_revise = ? WHERE id = ?";
    $procedural_statement_book = mysqli_prepare($mysqli_p, $query_book);
    mysqli_stmt_bind_param($procedural_statement_book, 'iii', $num_email, $status_email_revise, $bookval);
