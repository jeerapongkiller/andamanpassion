<?php
    ...
    if(!empty($_POST["prodval"]) && !empty($_POST["bp_pickup_time"])){
        $query = "UPDATE booking_products SET pickup_time = ? WHERE id = ?";
        $procedural_statement = mysqli_prepare($mysqli_p, $query);
        mysqli_stmt_bind_param($procedural_statement, 'si', $_POST["bp_pickup_time"], $_POST["prodval"]);
        mysqli_stmt_execute($procedural_statement);
        $result = mysqli_stmt_get_result($procedural_statement);

        $description_field = "เวลารับ : ".$_POST["bp_pickup_time"]." น.";
        $history_type = "2";
