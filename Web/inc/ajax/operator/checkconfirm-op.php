<?php
    ...
    if(!empty($_POST["prodval"]) && !empty($_POST["status"])){
        $query = "UPDATE booking_products SET status_confirm_op = ? WHERE id = ?";
        $procedural_statement = mysqli_prepare($mysqli_p, $query);
        mysqli_stmt_bind_param($procedural_statement, 'ii', $_POST["status"], $_POST["prodval"]);
        mysqli_stmt_execute($procedural_statement);
        $result = mysqli_stmt_get_result($procedural_statement);

        $description_field = $_POST["status"] == 1 ? "สถานะการยืนยัน(OP) : ยืนยันแล้ว" : "สถานะการยืนยัน(OP) : ยกเลิกการยืนยันแล้ว";
        $history_type = $_POST["status"] == 1 ? "5" : "6";
